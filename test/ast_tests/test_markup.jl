...
@testset "Markup inside a sentence for $m" for (m,T) in simple_markups
    ast = parse(Norg.AST.NorgDocument, "When put inside a sentence $(m)inner$(m).")
    nodes = collect(PreOrderDFS(ast))
...
@testset "Nested markup $n inside $m" for (m,T) in simple_nested_outer, (n,U) in simple_markups
    if m == n
        continue
    end
